...
class MoE(nn.Module):
    ...
    def __init__(self, input_size, experts, noisy_gating=True, k=4):
        ...
        for mdl in experts:
            mdl.eval()
        self.experts = nn.ModuleList([mdl for mdl in experts])
    ...
    def forward(self, x, loss_coef=1e-2):
        ...
        with torch.no_grad():
            expert_outputs = [self.experts[i](expert_inputs[i].view(expert_inputs[i].shape[0],  *original_shape[1:])) for i in range(self.num_experts)]
